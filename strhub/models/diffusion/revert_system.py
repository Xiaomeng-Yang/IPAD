...
from .modules import TokenEmbedding, Encoder, Decoder, DecoderLayerRevert
...
class DDPIMNet(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        decoder_layer = DecoderLayerRevert(embed_dim, dec_num_heads, time_step, embed_dim * dec_mlp_ratio, dropout)
    ...
    def decode(self, t: torch.Tensor, tgt: torch.Tensor, feature_map: torch.Tensor,
        ...
        if attn_mask is None:
            ...
            attn_mask = (tgt == self.mask_id)
            for row in range(tgt.shape[0]):
                if attn_mask[row, :].all():
                    attn_mask[row, :] = False
            attn_mask = attn_mask.unsqueeze(1).expand(-1, self.max_len, -1)
            inverted_identity = ~torch.eye(self.max_len, device=self._device, dtype=torch.bool)
            # unmask itself
            attn_mask = attn_mask * inverted_identity.unsqueeze(0).expand(t.shape[0], -1, -1)
            # expand for the head_number
            attn_mask = repeat(attn_mask, "bs L src_len -> (bs num_heads) L src_len",
                               num_heads=self.dec_num_heads)
        '''
        tgt = self.text_embed(tgt) + self.pos_embed
        # return self.decoder(t, tgt, feature_map, key_padding_mask, attn_mask=attn_mask)
    ...
    def iterative_decode(self, feature_map: torch.Tensor, input_labels: Optional[Tensor] = None,
        ...
        for i in range(self.num_iter):
            t = self.time_step - i * (self.time_step // self.num_iter)
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        if self.refine_iters:
            ...
            query_mask = torch.triu(torch.full((self.max_len, self.max_len), float('-inf'), device=self._device), 0)
            query_mask[torch.triu(torch.ones(self.max_len, self.max_len, dtype=torch.bool, device=self._device), 1)] = 0
            t = torch.full((bs,), 1, dtype=torch.long, device=self._device)
            for i in range(self.refine_iters):
                # Prior context is the previous output.
                tgt_in = logits.argmax(-1)
                tgt_padding_mask = ((tgt_in == self.eos_id).cumsum(-1) > 0)  # mask tokens beyond the first EOS token.
                tgt_out = self.decode(t, tgt_in, feature_map,
                                      key_padding_mask=tgt_padding_mask, attn_mask=None)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        loss_denoise = F.cross_entropy(logits, tgt.flatten(), ignore_index=self.pad_id)
        loss = loss_denoise + self.len_ratio * loss_len if self.len_token else loss_denoise
        self.log('loss', loss)
        return loss
