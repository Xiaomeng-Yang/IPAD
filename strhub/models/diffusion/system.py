...
class IPAD(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        # self.encoder.load_pretrained_weights('/home/test13/yxm/code/others/soft_attention/diffpimnet/saved_models/vit_small_checkpoint-19.pth')
    ...
    def _init_design_schedule(self):
        # store the transfer probabilities
        ct, att, btt, ctt = design_schedule(self.time_step, N=self.num_token - 2)   # designed schedule
        self.register_buffer('log_ct', torch.log(torch.tensor(ct)).float())
        self.register_buffer('log_att', torch.log(torch.tensor(att)).float())
        self.register_buffer('log_btt', torch.log(torch.tensor(btt)).float())
        self.register_buffer('log_ctt', torch.log(torch.tensor(ctt)).float())
    ...
    def decode(self, t: torch.Tensor, tgt: torch.Tensor, feature_map: torch.Tensor,
        ...
        tgt = self.dropout(self.text_embed(tgt) + self.pos_embed)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # tgt = torch.where(xt == self.mask_id, tgt, self.pad_id)
