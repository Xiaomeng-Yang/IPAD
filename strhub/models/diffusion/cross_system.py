...
from .modules import TokenEmbedding, Encoder, DecoderCross, DecoderLayerCross
...
class DDPIMNet(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        decoder_layer = DecoderLayerCross(embed_dim, dec_num_heads, time_step, embed_dim * dec_mlp_ratio, dropout)
        self.decoder = DecoderCross(decoder_layer, num_layers=dec_depth, norm=nn.LayerNorm(embed_dim))
        # initialize
    ...
    def decode(self, t: torch.Tensor, tgt: torch.Tensor, feature_map: torch.Tensor,
        ...
        pos_queries = self.pos_embed.expand(tgt.shape[0], -1, -1)
        tgt = self.text_embed(tgt) + self.pos_embed
        return self.decoder(t, tgt, pos_queries, feature_map, key_padding_mask, attn_mask=attn_mask)
