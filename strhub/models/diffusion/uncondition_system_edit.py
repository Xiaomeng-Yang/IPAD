...
class DDPIMNet(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        embed_len = (img_size[0]//patch_size[0]) * (img_size[1]//patch_size[1])
        self.pad_embed = nn.Parameter(torch.randn(size=(embed_len, embed_dim), requires_grad=True, dtype=torch.float32))
        # image free vector
    ...
    def iterative_decode(self, feature_map: torch.Tensor, input_labels: Optional[Tensor] = None,
        ...
        feature_map_uncondition = self.pad_embed
        feature_map_uncondition = feature_map_uncondition.unsqueeze(0).expand(bs, -1, -1)
        ...
        padding_factors = torch.tensor([1.0]*5, dtype=torch.float32, device=self._device)
        scaling_factors = torch.cat((self.scaling_factors, padding_factors), dim=0)
        ...
        for i in range(self.num_iter):
            ...
            out_uncondition = self.decode(t, tgt_tokens, feature_map_uncondition)

            new_token_logits = self.head(outputs).float()
            new_token_uncondition = self.head(out_uncondition).float()
            new_token_logits = (1 - scaling_factors[t-1]).view(-1, 1, 1) * new_token_uncondition \
                + scaling_factors[t-1].view(-1, 1, 1) * new_token_logits
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # empty_idx = int(self.drop_rate * bs)
        # for i_image in range(empty_idx):
        #     feature_map[i_image, :, :] = self.pad_embed
        feature_map_uncondition = self.pad_embed
        feature_map_uncondition = feature_map_uncondition.unsqueeze(0).expand(bs, -1, -1)
        key_padding_mask = (tgt == self.pad_id) | (tgt == self.eos_id)
        out = self.decode(t, xt, feature_map, key_padding_mask=key_padding_mask)
        out_uncondition = self.decode(t, xt, feature_map_uncondition, key_padding_mask=key_padding_mask)
        ...
        logits = (1 - scaling_factors[t]).view(-1, 1, 1) * logits_uncondition + \
            scaling_factors[t].view(-1, 1, 1) * logits
