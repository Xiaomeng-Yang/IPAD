...
from .modules import TokenEmbedding, Encoder, DecoderWot, DecoderLayerWot
...
class DDPIMNet(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        decoder_layer = DecoderLayerWot(embed_dim, dec_num_heads, time_step, embed_dim * dec_mlp_ratio, dropout)
        self.decoder = DecoderWot(decoder_layer, num_layers=dec_depth, norm=nn.LayerNorm(embed_dim))
        # initialize
    ...
    def decode(self, tgt: torch.Tensor, feature_map: torch.Tensor,
        ...
        return self.decoder(tgt, feature_map, key_padding_mask, attn_mask=None)
        # return self.decoder(tgt, feature_map, key_padding_mask, attn_mask=None)
    ...
    def iterative_decode(self, feature_map: torch.Tensor, input_labels: Optional[Tensor] = None,
        ...
        for i in range(self.num_iter):
            ...
            # outputs = self.decoder(t, predicts_embed, feature_map, key_padding_mask, attn_mask=None)
            outputs = self.decoder(predicts_embed, feature_map, key_padding_mask, attn_mask=None)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        if self.len_token:
            ...
            # _, len_pred = torch.max(len_logits.softmax(-1), 1)
            # the length loss
            loss_len = F.cross_entropy(len_logits, len_gt)
            feature_map = feature_map[:, 1:]
        ...
        # t = t.unsqueeze(dim=1)
        out = self.decode(xt, feature_map, key_padding_mask=key_padding_mask)
