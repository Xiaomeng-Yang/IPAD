...
from .modules import Encoder, TokenEmbedding, Decoder, DecoderLayer
...
class MPNet(BaseSystem):
    ...
    def iterative_decode(self, feature_map: torch.Tensor, target: Optional[Tensor] = None,
        ...
        pred_tgt_tokens = torch.full((bs, self.max_label_length + 1), self.pad_id, dtype=torch.long, device=self._device)
        token_logits = torch.zeros([bs, self.max_label_length + 1, len(self.tokenizer) - 2], dtype=torch.float, device=self._device)
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None, target: Optional[Tensor] = None) -> Tensor:
        ...
        padding_mask = torch.full((bs, num_steps+1), False, dtype=torch.bool, device=self._device)
        for i in range(len(length_pred)):
            padding_mask[i, length_pred[i]+2:] = True
            tgt_padding_mask[i, length_pred[i]+2:] = True
        ...
        if self.decode_ar:
            ...
            for i in range(num_steps):
                ...
                tgt_out = self.decode(tgt_in[:, :j], memory[:, 1:], tgt_mask[:j, :j],
        ...
        if self.refine_iters:
            ...
            query_mask_extend[torch.triu(torch.ones(num_steps, num_steps+1, dtype=torch.bool, device=self._device), 2)] = float('-inf')
            ...
            for i in range(self.refine_iters):
                ...
                tgt_padding_mask = torch.cat([torch.full((bs, 1), False, device=self._device), tgt_padding_mask[:,:-1]], dim=1)
                if target is not None:
                   tgt_padding_mask = (target == self.pad_id)[:, :tgt_in.shape[1]]
                tgt_out = self.decode(tgt_in, memory[:, 1:], tgt_mask,
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        # tgt_in = tgt[:, :-1]    # not include the [EOS] token ['BOS', TOKEN, TOKEN, 'EOS', 'PAD, 'PAD']
        ...
        eos_idx = torch.eq(tgt, self.eos_id).nonzero()
        disturb_idx = random.sample(range(bs), bs//3)
        for idx in disturb_idx:
            type_dis = random.choice(range(2))
            if type_dis == 0:
                # unpad the pad after the eos token (predict length + 1), change pad_id to mask
                if (eos_idx[idx][1] + 1) < tgt_extend.shape[1]:  
                    tgt_extend[idx][eos_idx[idx][1] + 1] = self.mask_id
            else:
                # pad the eos token (predict length - 1)
        ...
        length_gt = torch.full((bs, tgt.shape[1] - 1), 0.0, dtype=torch.float, device=self._device)
        length_gt = torch.where(tgt_out==self.eos_id, 1.0, length_gt)

        # decode mlm
        if not self.decode_ar:
            ...
            logits, _, _ = self.iterative_decode(feature[:, 1:], target=tgt_out)
            loss += n * F.cross_entropy(logits.flatten(end_dim=1), tgt_out.flatten(), ignore_index=self.pad_id)
        ...
        if self.use_gt: 
            for i, perm in enumerate(tgt_perms):
                ...
                out = self.decode(tgt_in, feature[:, 1:], tgt_mask, tgt_padding_mask, tgt_query_mask=query_mask)
                # out = self.decode(tgt_in, feature, tgt_mask, tgt_padding_mask, tgt_query_mask=query_mask)
