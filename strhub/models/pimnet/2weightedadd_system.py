...
class PIMNet(BaseSystem):
    def __init__(self, charset_train: str, charset_test: str, max_label_length: int,
        ...
        self.alpha = nn.Parameter(torch.tensor(0.5))    # learnable fusion ratio
    ...
    def iterative_decode(self, feature_map: torch.Tensor, input_labels=None):
        ...
        second_tgt_tokens = torch.full((bs, self.max_len), self.mask_id, dtype=torch.long, device=self._device)
        pred_tgt_tokens = torch.full((bs, self.max_len), self.mask_id, dtype=torch.long, device=self._device)
        token_logits = torch.zeros([bs, self.max_len, len(self.tokenizer) - 1],
        ...
        for i in range(self.num_iter):
            ...
            key_padding_mask2 = ((second_tgt_tokens == self.eos_id).cumsum(-1) > 0)
            # the final padding_mask is the joint of the two
            key_padding_mask = key_padding_mask & key_padding_mask2
            ...
            predicts_embed = self.text_embed(tgt_tokens)
            predicts_embed2 = self.text_embed(second_tgt_tokens)
            predicts_embed = self.alpha * predicts_embed + (1 - self.alpha) * predicts_embed2 + self.pos_embed
            ...
            new_tgt_tokens = torch.topk(new_token_logits, k=2, dim=-1).indices
            ...
            logits_update_idx = torch.tile(update_idx.unsqueeze(dim=2), [1, 1, len(self.tokenizer) - 1])
            ...
            if input_labels is not None:    # is training
                ...
                second_tgt_tokens = torch.where(update_idx, input_labels, second_tgt_tokens)
                pred_tgt_tokens = torch.where(update_idx, new_tgt_tokens[:, :, 0], pred_tgt_tokens)

            else:   # testing
                tgt_tokens = torch.where(update_idx, new_tgt_tokens[:, :, 0], tgt_tokens)
                second_tgt_tokens = torch.where(update_idx, new_tgt_tokens[:, :, 1], second_tgt_tokens)
