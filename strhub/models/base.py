...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def _eval_step(self, batch, validation: bool, Chinese:bool) -> Optional[STEP_OUTPUT]:
        images, labels = batch
        ...
        label_length = 0
        if validation:
            return_values = self.forward_logits_loss(images, labels)
            if len(return_values) == 4:
                pad_pred, logits, loss, loss_numel = return_values
            else:
                logits, loss, loss_numel = return_values
        else:
            ...
            logits = self.forward(images)
            # _, logits, loss, loss_numel = self.forward_logits_loss(images, labels)
        ...
        if Chinese:
            for pred, prob, gt in zip(preds, probs, labels):
                ...
                # for chinese recog, convert it
                gt = zhconv.convert(strQ2B(gt), 'zh-cn').lower()
                pred = zhconv.convert(strQ2B(pred), 'zh-cn').lower()
            ...
            for pred, prob, gt in zip(preds, probs, labels):
                ...
                total += 1
                label_length += len(pred)
        return dict(output=BatchResult(total, l_correct, correct, ned, confidence, label_length, loss, loss_numel,
                                       pred_list, gt_list))
    ...
    def validation_step(self, batch, batch_idx) -> Optional[STEP_OUTPUT]:
        return self._eval_step(batch, True, False)
    ...
    def test_step(self, batch, batch_idx, Chinese=False) -> Optional[STEP_OUTPUT]:
        return self._eval_step(batch, False, Chinese)
