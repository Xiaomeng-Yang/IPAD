...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root: str, charset: str, max_label_len: int, min_image_dim: int = 0,
                 ...
                 unlabelled: bool = False, transform: Optional[Callable] = None, train: bool = True):
        self._env = None
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        '''
        origin_img = Image.open(buf).convert('RGB')
        trans = T.ToTensor()
        if self.transform is not None:
            img = self.transform(origin_img)

        origin_img = trans(origin_img)
        return origin_img, img, label
        '''
