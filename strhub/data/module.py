...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    # TEST_BENCHMARK = ('ArT', 'COCOv1.4', 'Uber')
    TEST_BENCHMARK_SUBADD = ('IIIT5k', 'SVT', 'IC13_857', 'IC15_1811', 'SVTP', 'CUTE80', 'TotalText', 'CTW')
    TEST_BENCHMARK_ADD = ('IIIT5k', 'SVT', 'IC13_1015', 'IC15_2077', 'SVTP', 'CUTE80', 'TotalText', 'CTW')
    TEST_HARDCASES = ('bg_confusion','blur','oblique_or_curved','occlusion','vertical_text')
    TEST_OTHER = ('WordArT', 'OST_weak', 'OST_heavy')
    TEST_NEW = ('ArT', 'COCOv1.4', 'Uber')
    TEST_CHINESE = ('scene', 'web', 'document', 'handwriting')
    TEST_UNION = ('curve','multi_oriented','artistic','contextless','salient','multi_words','general')
    TEST_ALL = tuple(set(TEST_BENCHMARK_SUB + TEST_BENCHMARK + TEST_NEW + TEST_OTHER))
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            ...
            self._train_dataset = build_tree_dataset(root, self.charset_train, self.max_label_length,
                                                     ...
                                                     transform=transform, train=self.train)
    ...
    def test_dataloaders(self, subset):
        ...
        datasets = {s: LmdbDataset(str(root / s), self.charset_test, self.max_label_length,
                                   self.min_image_dim, self.remove_whitespace, self.normalize_unicode,
                                   transform=transform, train=self.train) for s in subset}
        return {k: DataLoader(v, batch_size=self.batch_size, num_workers=self.num_workers,
                              pin_memory=True, collate_fn=self.collate_fn)
                for k, v in datasets.items()}

    def test_dataloader_chinese(self):
        transform = self.get_transform(self.img_size, rotation=self.rotation)
        root = PurePath(self.root_dir)
        dataset = LmdbDataset(str(root), self.charset_test, self.max_label_length,
                               self.min_image_dim, self.remove_whitespace, False,
                               transform=transform, train=self.train)
        return DataLoader(dataset, batch_size=self.batch_size, num_workers=self.num_workers,
                          pin_memory=True, collate_fn=self.collate_fn)
                          
                          
    def test_dataloaders_chinese(self, subset):
        transform = self.get_transform(self.img_size, rotation=self.rotation)
        root = PurePath(self.root_dir)
        datasets = {s: LmdbDataset(str(root / s), self.charset_test, self.max_label_length,
                                   self.min_image_dim, self.remove_whitespace, False,
