...
def main(config: DictConfig):
    ...
    if config.ckpt_path is not None:
        model: BaseSystem = load_from_checkpoint(config.ckpt_path)
    else:
        model: BaseSystem = hydra.utils.instantiate(config.model)
    ...
    checkpoint = ModelCheckpoint(monitor='val_accuracy', mode='max', save_top_k=5, save_last=True,
                                 filename='{epoch}-{step}-{val_accuracy:.4f}-{val_NED:.4f}')
    swa = StochasticWeightAveraging(swa_epoch_start=0.85, swa_lrs=1e-3)
