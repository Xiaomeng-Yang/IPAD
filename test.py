...
def main():
    ...
    parser.add_argument('--other', action='store_true', default=False, help='Evaluate on other benchmark datasets')
    ...
    #     SceneTextDataModule.TEST_BENCHMARK_ADD + SceneTextDataModule.TEST_BENCHMARK_SUBADD
    # test_set = SceneTextDataModule.TEST_BENCHMARK_SUB
    test_set = SceneTextDataModule.TEST_BENCHMARK_SUB + SceneTextDataModule.TEST_BENCHMARK
    if args.new:
        test_set += SceneTextDataModule.TEST_NEW
    if args.other:
        test_set = SceneTextDataModule.TEST_OTHER
    ...
    if args.store_result:
        output_file = open(args.checkpoint+'_results.txt', 'w', encoding='utf-8')
        output_file.write('GT\tPredict\n')
        output_dir = os.path.join('./visualize', 'image')
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
            
    # correct_len = {3: 0, 4: 0, 5: 0, 6: 0, 7: 0, 8: 0, 9: 0, 10: 0}
    # total_len = {3: 0, 4: 0, 5: 0, 6: 0, 7: 0, 8: 0, 9: 0, 10: 0}
    ...
    for name, dataloader in datamodule.test_dataloaders(test_set).items():
        ...
        confidence = 0
        label_length = 0
        # for ori_imgs, imgs, labels in tqdm(iter(dataloader), desc=f'{name:>{max_width}}'):
        for imgs, labels in tqdm(iter(dataloader), desc=f'{name:>{max_width}}'):
            ...
            '''
            if args.store_result:
                for i in range(len(res.gt_list)):
                    if res.gt_list[i] != res.pred_list[i]:
                        output_file.write(res.gt_list[i] + '\t' + res.pred_list[i] + '\n')
                        img_name = str(idx) + '-' + res.gt_list[i] + '-' + res.pred_list[i] + '.png'
    ...
    result_groups = {
        ...
        # 'BenchmarkAdd (Subset)': SceneTextDataModule.TEST_BENCHMARK_SUBADD,
        # 'BenchmarkAdd': SceneTextDataModule.TEST_BENCHMARK_ADD
    }
    ...
    if args.other:
        result_groups = {'Other': SceneTextDataModule.TEST_OTHER,}
