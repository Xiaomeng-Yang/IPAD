...
def mpnet(pretrained: bool = False, decode_ar: bool = True, refine_iters: int = 1, **kwargs):
    """
    MPNet base model (img_size=128x32, patch_size=8x4, d_model=384)
    @param pretrained: (bool) Use pretrained weights
    @param decode_ar: (bool) use AR decoding
    """
    return create_model('mpnet', pretrained, decode_ar=decode_ar, refine_iters=refine_iters, **kwargs)
...
def diffusion(pretrained: bool = False, num_iter: int = 5, **kwargs):
    """
    ABINet model (img_size=128x32)
    @param pretrained: (bool) Use pretrained weights
    @param num_iter: (int) number of refinement iterations to use
    """
    return create_model('diffusion', pretrained, num_iter=num_iter, **kwargs)
